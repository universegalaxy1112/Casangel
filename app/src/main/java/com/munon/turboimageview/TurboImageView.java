...
public class TurboImageView extends View implements MultiTouchObjectCanvas<MultiTouchObject> {
    ...
    public interface TurboImageViewEventListener {
        public void allObjectsDeselected();
    }

    public TurboImageViewEventListener getTurboImageViewEventListener() {
        return turboImageViewEventListener;
    }

    public void setTurboImageViewEventListener(TurboImageViewEventListener turboImageViewEventListener) {
        this.turboImageViewEventListener = turboImageViewEventListener;
    }

    private TurboImageViewEventListener turboImageViewEventListener;
    ...
    public ArrayList<MultiTouchObject> getObjects() {
        return mImages;
    }

    public void loadObjects(ArrayList<MultiTouchObject> mImages) {
        deselectAll();
        this.mImages.clear();
...
//            mImages.add(multiTouchObject);
    ...
    @Override
    public void removeObject(MultiTouchObject multiTouchObject, PointInfo touchPoint) {
        currTouchPoint.set(touchPoint);
        if (multiTouchObject != null) {
            // Move image to the top of the stack when selected
            mImages.remove(multiTouchObject);
    ...
    @Override
    public void deselectObject(MultiTouchObject multiTouchObject, PointInfo touchPoint) {
        currTouchPoint.set(touchPoint);
        if (multiTouchObject != null) {
            // Move image to the top of the stack when selected
            multiTouchObject.isLatestSelected = false;
        }
        invalidate();
    }

    @Override
    public void flipObject(MultiTouchObject multiTouchObject, PointInfo touchPoint) {
        currTouchPoint.set(touchPoint);
        if (multiTouchObject != null) {
            multiTouchObject.setFlippedHorizontally(!multiTouchObject.isFlippedHorizontally());
        }
    ...
    @Override
    public void aheadObject(MultiTouchObject multiTouchObject, PointInfo touchPoint) {
        currTouchPoint.set(touchPoint);
        if (multiTouchObject != null) {

            if (mImages.size() > 1) {

                ArrayList<MultiTouchObject> tempObjects = new ArrayList();

                for (int i = 0; i < mImages.size(); i++) {
                    if (multiTouchObject == mImages.get(i)) {
                        if (i < mImages.size() - 1) {

                            tempObjects.add(mImages.get(i + 1));
                            tempObjects.add(mImages.get(i));
                            i++;
                        } else {
                            tempObjects.add(mImages.get(i));
                        }
    ...
    @Override
    public void behindObject(MultiTouchObject multiTouchObject, PointInfo touchPoint) {
        currTouchPoint.set(touchPoint);
        if (multiTouchObject != null) {
                ...
                for (int i = 0; i < mImages.size(); i++) {
                    if (multiTouchObject == mImages.get(i)) {
                        if (i > 0) {
                            ...
                            tempObjects.remove(mImages.get(i - 1));
    ...
    public void canvasTouched() {
        ...
        if (turboImageViewEventListener != null) {
            turboImageViewEventListener.allObjectsDeselected();
        }
    }
